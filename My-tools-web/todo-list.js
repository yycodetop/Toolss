/**
 * Task Command Module - Timeline Expansion Edition
 * 升级：增加无限延伸的时间轴，可自由加载更早的历史任务和未来的排期任务
 */
        ...
        <div v-else class="flex-1 flex flex-col relative bg-[#0b0c15] overflow-hidden">
            <div class="h-10 border-b border-white/10 flex bg-[#0f111a] shrink-0 overflow-hidden relative select-none">
                <div class="flex" :style="{ transform: 'translateX(-' + scrollX + 'px)' }">
                    <button @click="loadEarlier" class="w-[50px] shrink-0 border-r border-white/5 flex flex-col items-center justify-center text-gray-500 hover:text-cyan-400 hover:bg-white/5 group transition-colors cursor-pointer" title="加载更早记录">
                    ...
                    <button @click="loadLater" class="w-[50px] shrink-0 border-l border-white/5 flex flex-col items-center justify-center text-gray-500 hover:text-cyan-400 hover:bg-white/5 group transition-colors cursor-pointer" title="向后延伸时间轴">
                        <i class="fa-solid fa-angles-right group-hover:translate-x-1 transition-transform"></i>
                    </button>
                </div>
            </div>
            
            <div class="flex-1 overflow-auto custom-scrollbar relative" @scroll="handleScroll" ref="timelineScrollContainer">
                <div class="absolute inset-0 flex pointer-events-none h-full" :style="{ width: (timelineDays.length * 100 + 100) + 'px' }">
                    <div class="w-[50px] shrink-0 border-r border-white/5 h-full bg-white/[0.01]"></div>
                    ...
                    <div class="w-[50px] shrink-0 border-l border-white/5 h-full bg-white/[0.01]"></div>
                </div>
                
                <div class="relative py-4 space-y-4 min-h-full" :style="{ width: (timelineDays.length * 100 + 100) + 'px' }">
                    <div v-if="scheduledTasks.length === 0" class="absolute left-16 top-10 text-gray-600 text-sm flex items-center gap-2"><i class="fa-solid fa-timeline"></i> 暂无进行中的任务，请从右上角待办池拖入或新建</div>
...
const TodoListComponent = {
    ...
    setup() {
        ...
        const timelineStartOffset = ref(-14); // 默认过去两周
        const timelineEndOffset = ref(45);
        ...
        const generateTimeline = () => { 
            ...
            const t=new Date(); 
            t.setHours(0,0,0,0);
            for(let i=timelineStartOffset.value; i<timelineEndOffset.value; i++){ 
                const d=new Date(t); 
                d.setDate(t.getDate()+i); 
                arr.push({date:d, label:`${d.getMonth()+1}/${d.getDate()}`, week:['日','一','二','三','四','五','六'][d.getDay()]}); 
            } 
            timelineDays.value=arr; 
        };

        const loadEarlier = () => {
            timelineStartOffset.value -= 30; // 往前加载 30 天
            generateTimeline();
            // 保持用户的视觉焦点，将新增加的宽度补充到 scrollLeft
            setTimeout(() => {
                if (timelineScrollContainer.value) {
                    timelineScrollContainer.value.scrollLeft += 3000;
                }
            }, 0);
        };

        const loadLater = () => {
            timelineEndOffset.value += 30; // 往后加载 30 天
            generateTimeline();
        };
        ...
        const getTaskStyle = (task) => { 
            ...
            s.setHours(0,0,0,0);
            const e = task.end ? new Date(task.end) : new Date(s); 
            e.setHours(0,0,0,0);
            
            if (timelineDays.value.length === 0) return {};
            const baseDate = new Date(timelineDays.value[0].date); 
            baseDate.setHours(0,0,0,0);
            
            const off = Math.round((s - baseDate) / 86400000); 
            let dur = Math.round((e - s) / 86400000) + 1; 
            if(dur < 1) dur = 1; 
            // left 加 50px 是为了给左侧 "加载更早" 留出的空白区域
            return { left: (50 + off * 100) + 'px', width: (dur * 100) + 'px' }; 
        };
        ...
        return { 
            ...
            statDetailUser, timelineScrollContainer,
